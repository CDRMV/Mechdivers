...
# Terran Nuke Missile
#
local TIFMissileNuke = import('/lua/sim/defaultprojectiles.lua').SingleBeamProjectile
...
TIFICBM = Class(TIFMissileNuke) {
    ...
    ThrustFireEffect = {
					 '/mods/Mechdivers/effects/emitters/icbm_trail_01_emit.bp',
					 '/mods/Mechdivers/effects/emitters/icbm_trail_01_emit.bp',
    },
	...
	CreateFireEffects = function( self, EffectTable, army, scale, Length)
        for k, v in EffectTable do
            self.Trash:Add(CreateAttachedEmitter(self, -1, army, v):ScaleEmitter(scale):SetEmitterCurveParam('LIFETIME_CURVE', 10, 1))
        end
    end,
    ...
    MovementThread = function(self)
		...
		self.CreateFireEffects( self, self.ThrustFireEffect, army, 1.5, 3 )
...
TIFICBM = Class(TIFMissileNuke) {
    ...
    MovementThread = function(self)
		...
		self.CreateEffects( self, self.LaunchEffects, army, 1 )
		self.CreateEffects( self, self.ThrustEffects, army, 3 )
		self.CreateFireEffects( self, self.ThrustFireEffect, army, 1.2, 3 )
        WaitSeconds(2.5)		# Height
        self:SetCollision(true)
        WaitSeconds(2.5)
        self:TrackTarget(true) # Turn ~90 degrees towards target
