...
function SetLayout(layout)
    import('/mods/Mechdivers/UI/Layout/MainPanel_Layout.lua').SetLayout()
    CommonLogic()
end
    ...
    if UIUtil.GetAnimationPrefs() then
        if state or controls.bg:IsHidden() then
            ...
            controls.bg.OnFrame = function(self, delta)
                ...
                if newLeft > savedParent.Left()+78 then
                    newLeft = savedParent.Left()+78
            ...
            controls.bg.OnFrame = function(self, delta)
                local newLeft = self.Left() - (1000*delta)
                if newLeft < savedParent.Left()-self.Height() then
                    newLeft = savedParent.Left()-self.Height() 
                    self:SetNeedsFrameUpdate(false)
                    self:Hide()
                end
                self.Left:Set(newLeft)
