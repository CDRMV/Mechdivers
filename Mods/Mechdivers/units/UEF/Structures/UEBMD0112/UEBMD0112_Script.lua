...
UEBMD0101 = Class(TStructureUnit) {
	...
	OnStopBeingBuilt = function(self,builder,layer)
		...
		if not self.MastAnimationManipulator then
            self.MastAnimationManipulator = CreateAnimator(self)
            self.Trash:Add(self.MastAnimationManipulator)
        end
        self.MastAnimationManipulator:PlayAnim(self:GetBlueprint().Display.AnimationMast, false):SetRate(0)	
		if not self.FlagAnimationManipulator then
            self.FlagAnimationManipulator = CreateAnimator(self)
            self.Trash:Add(self.FlagAnimationManipulator)
        end
        self.FlagAnimationManipulator:PlayAnim(self:GetBlueprint().Display.AnimationFlag, true):SetRate(0.5)	
		...
		self:ShowBone( 'Mast', true )
		self:HideBone( 'Flag1', true )
	...
	CaptureThread = function(self)
		local army = self:GetArmy()
 		while not self:IsDead() do
			...
			if SelectedUnit() == self then
			DrawCircle(self:GetPosition(), 5, 'ff0000')
			end
            #Get Enemy units in the area
			local units = self:GetAIBrain():GetUnitsAroundPoint(categories.MOBILE + categories.LAND, unitPos, 15, 'Ally')
			local enemyunits = self:GetAIBrain():GetUnitsAroundPoint(categories.MOBILE + categories.LAND, unitPos, 15, 'Enemy')
			if units[1] then
				self.MastAnimationManipulator:SetRate(0.02)	
				if self.MastAnimationManipulator:GetAnimationFraction() == 1 then
					self:ShowBone( 'Flag1', true )
					break
				end
			end	
			
			if units[1] and enemyunits[1] or enemyunits[1] then
				self.MastAnimationManipulator:SetRate(0)
			end
