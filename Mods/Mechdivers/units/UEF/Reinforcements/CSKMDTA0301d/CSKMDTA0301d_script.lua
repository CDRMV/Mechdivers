...
CSKMDTA0301d = Class(TAirUnit) {
    ...
    OnStopBeingBuilt = function(self,builder,layer)
		...
		self.unit = CreateUnitHPR('CSKMDTL0305', self:GetArmy(), position[1], position[2], position[3], 0, 0, 0)
		self.unit:AttachBoneTo('AttachPoint', self, 'SpawnAttachpoint3')
		local MainGun = self.unit:GetWeaponByLabel('MainGun')
		MainGun:SetEnabled(false)
		local MachineGun = self.unit:GetWeaponByLabel('MachineGun')
		MachineGun:SetEnabled(false)
		local GatlingCannon = self.unit:GetWeaponByLabel('GatlingCannon')
		GatlingCannon:SetEnabled(false)
	...
	OnTransportDetach = function(self, attachBone, unit)
		...
		local MachineGun = unit:GetWeaponByLabel('MachineGun')
		MachineGun:SetEnabled(true)
		local GatlingCannon = unit:GetWeaponByLabel('GatlingCannon')
		GatlingCannon:SetEnabled(true)
		local position = self:GetPosition()
