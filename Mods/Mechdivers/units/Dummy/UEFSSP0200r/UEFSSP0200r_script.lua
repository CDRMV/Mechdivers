...
UEFSSP0200r= Class(StructureUnit) {
    ...
    OnStopBeingBuilt = function(self,builder,layer)
        StructureUnit.OnStopBeingBuilt(self,builder,layer)
			...
			self.Effect1 = CreateAttachedEmitter(self,0,self:GetArmy(), Effectpath .. 'miasma_cloud_01_emit.bp'):ScaleEmitter(1.0):SetEmitterParam('LIFETIME', -1)
			self.Effect2 = CreateAttachedEmitter(self,0,self:GetArmy(), Effectpath .. 'miasma_cloud_01_emit.bp'):ScaleEmitter(1.0):SetEmitterParam('LIFETIME', -1)
				local interval = 0
                while (interval < 6) do
				LOG(interval)
					if interval < 5 then 
